...
class ClYoloUpUp(nn.Module):
    cell_size = [8, 8]
    specs = {"cell_size": cell_size}

    def __init__(self, args, coords):
        super(ClYoloUpUp, self).__init__()
        ...
        self.yolo = nn.Conv2d(512, self.output_channels, 1, 1, 0, bias=True)
...
if __name__ == "__main__":
    ...
    args = general_setup(name="YOLOv1 2x up", config_file="darknet_params.yaml", setup_logging=False,
                         ignore_cmd_args=True,
                         alternative_args={"model": "clyoloupup"})

    model = ClYoloUpUp(num_predictors=args.num_predictors, num_coords=5, darknet_cfg=args.darknet_cfg,
                       darknet_weights=args.darknet_weights)
    inp = get_test_input((320, 640, 3), args.batch_size)
    pred = model(inp)  # torch.Size([8, 3200, 8, 5]) => 40x80
